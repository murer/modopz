...
public class MODataSocketCommand implements MOCommand {
	...
	public MOSocketResult execute(MOKernel kernel) {
		...
		if (socket == null) {
			throw new MOMessageException("not found", new MOSocketNotFoundResult().setId(id));
		}
		...
		if (read == null) {
			...
			throw new MOMessageException("not found", new MOSocketNotFoundResult().setId(socket.getId()));
		}
